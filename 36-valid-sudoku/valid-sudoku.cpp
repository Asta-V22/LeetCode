        ...
        for(int i=0; i<9; i++){
            for(int j=0; j<9; j++){
                if(board[i][j]!='.'){
                    ...
                    //commented lines also work but the current one is a smarter one
                    ...
                    if(!(cols[j].insert(board[i][j]).second)) return false;
                    ...
                    if(!(boxs[numBox].insert(board[i][j]).second)) return false;
                }
            }
        }
